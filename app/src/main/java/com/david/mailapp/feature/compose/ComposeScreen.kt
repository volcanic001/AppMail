    ...
    Scaffold(
        ...
        topBar = {
            ComposeTopBar(
                ...
                sendEnabled = !uiState.isLoadingOriginalEmail && uiState.originalEmailError == null
            )
        },
        snackbarHost = { SnackbarHost(snackbarHostState) }
    ) { paddingValues ->
        if (uiState.isLoadingOriginalEmail) {
            Box(
                modifier = Modifier
                    .fillMaxSize()
                ...
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    ...
                    Text(
                        text = stringResource(R.string.compose_loading_original),
                        style = MaterialTheme.typography.bodyMedium,
                        color = MaterialTheme.colorScheme.onSurfaceVariant
                    )
                }
            }
        } else if (uiState.originalEmailError != null) {
            val errorText = when (uiState.originalEmailError) {
                UiErrorReason.EMAIL_NOT_FOUND -> stringResource(R.string.compose_error_email_not_found)
            ...
            Box(
                ...
                Column(
                    ...
                    Button(onClick = onClose) {
                        Text(stringResource(R.string.compose_btn_go_back))
                    }
                }
            }
        } else {
